...
class File:
    ...
    def Ui_graph_channel2(self): ## UI for graph 2
    ...
    def Ui_graph_channel1(self): ## UI for graph 1
        print("graph1")
    ...
    def animate_fig2(self, i):  ## To animate graph 2
        ...
        for idx_line_ch2 in self.dic_channel2.items():   ## iterate on lines ,dic={line:data of line,...}
            ...
            if idx_line_ch2[0] != 0 and idx_line_ch2[0]  in self.present_line2.keys():  ## check if line in present lines or not
                ...
                for idx_line2 in self.present_line2.items():
                    if idx_line2[0] == idx_line_ch2[0]:
                        ...
                        self.x_fig2[idx_line_ch2[0]].append(idx_line_ch2[1][0][self.specific_row_2 -(current_idx_2 + 1)])
                        self.y_fig2[idx_line_ch2[0]].append(idx_line_ch2[1][1][self.specific_row_2 -(current_idx_2 + 1)])
            ...
            if self.link:  ## check if two graphs liked or not
                ...
                if self.ax.get_navigate_mode() == 'ZOOM' or  self.ax.get_navigate_mode() == 'PAN'  :      ## check which button is clicked
    ...
    def animate_fig1(self,i): ## to animate graph1
        ...
        for idx_line_ch1 in self.dic_channel1.items():           ## iterate on lines ,dic={line:data of line,...}
            ...
            if self.link:  ## check if two graphs liked or not
                ...
                if self.ax2.get_navigate_mode() == 'ZOOM' or self.ax2.get_navigate_mode() == 'PAN':  ## check which button is clicked
       ...
       ## get range of data of signal
        ...
        if channel1 == "None" and file_part in self.visited_channel1:
            ...
            for idx_line_ch1 in self.dic_channel1.items():
                self.lines1[idx_line_ch1[0]], = self.ax.plot([], [], label=self.name_files_ch1[idx_line_ch1[0]],color=self.colors_channel1[idx_line_ch1[0]])
        ...
        if channel1 == "channel1":
            ...
            if no_of_repeated == 1:
                ...
                if self.no_of_line > 1 and len(self.visited_channel1) != 1:  ## to add another lines and begin  from last time of first line
                ...
                if graph_ch1: ## show graph on graphics view
                    ...
                    # self.Qwindow.button_zoom_out.show()
       ...
       ## channel2 /graph 2
        if channel2 == "None" and file_part in self.visited_channel2:
            ...
            for idx_line_ch2 in self.dic_channel2.items():
                self.lines2[idx_line_ch2[0]], = self.ax2.plot([], [], label=self.name_files_ch2[idx_line_ch2[0]] ,color=self.colors_channel2[idx_line_ch2[0]])
